...
check(server.includes('cleanDataDir();') && server.includes('actual.init(initConfig)') && server.includes('actual.downloadBudget(budgetId)') && server.includes('actual.loadBudget(budgetId)'), 'server.js worker should initialize, download, and load Actual budgets before API operations');
